...
{
	if (latencies.empty() ||
	    std::any_of(latencies.begin(), latencies.end(), [](double latency) { return !std::isfinite(latency); })) {
...
double calculate_speedup_ratio(double baseline_average_ms, double candidate_average_ms)
{
	if (!std::isfinite(baseline_average_ms) || baseline_average_ms < 0.0 || !std::isfinite(candidate_average_ms) ||
	    candidate_average_ms <= 0.0) {
		throw std::invalid_argument(
			"latency averages must have a finite nonnegative baseline and finite positive candidate");
	}

	const double ratio = baseline_average_ms / candidate_average_ms;
	if (!std::isfinite(ratio)) {
		throw std::invalid_argument("speedup ratio must be finite");
	}
	return ratio;
}
				 ...
				 std::size_t foreground_channel, std::size_t channel_count, float threshold)
{
	if (cpu_output.empty() || cpu_output.size() != candidate_output.size() || channel_count == 0 ||
	    foreground_channel >= channel_count || cpu_output.size() % channel_count != 0 ||
	    !std::isfinite(threshold)) {
		throw std::invalid_argument(
			"outputs must have matching non-empty interleaved channels and a finite threshold");
	}
	...
	for (std::size_t index = 0; index < cpu_output.size(); ++index) {
		...
		absolute_error += std::fabs(static_cast<double>(cpu_output[index]) -
					    static_cast<double>(candidate_output[index]));
	...
	return {.mean_absolute_error = static_cast<float>(absolute_error / static_cast<double>(cpu_output.size())),
		...
		.foreground_iou =
			union_count == 0 ? 1.0F : static_cast<float>(intersection) / static_cast<float>(union_count)};
}
